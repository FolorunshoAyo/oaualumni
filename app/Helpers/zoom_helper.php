...
function createMeeting($data){
    ...
    */
    ...
    if ($result && isset($result['id']) && isset($result['start_url'])) {
        return array('meeting_id' => $result['id'], 'meeting_url' => $result['start_url']);
    } else {
        return false;
    }    
}
...
function newAccessToken(){
    ...
    $headers = array(
        "Authorization: Basic " . base64_encode(getenv('ZOOM_CLIENT_ID') . ':' . getenv('ZOOM_CLIENT_SECRET')),
        "Host: zoom.us"
    );
