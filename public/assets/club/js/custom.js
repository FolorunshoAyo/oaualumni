(function($) {
    ...
    $('#image-slider').layerSlider({
        ...
        skinsPath: baseurl+'/public/assets/club/skins/',
        height: 900
    });
    ...
    $(function(){
        $('.skzsbt').click(function(){
            var userEmail = jQuery.trim($('#sbtxet').val());
            var spiner='';
            ...
            $(".subfdbk").html(spiner);
            if (userEmail == "") {
                $('.subfdbk').html(customError('alert-warning','Email field is required'));
            }
            else{

                if (!validemail(userEmail)) {
                    $('.subfdbk').html(customError('alert-warning','This is not a valid email address.'));
                }
                else{
                    $.ajax({
                        url:sturl+'user/subscribe',
                        type:'post',
                        ...
                        success:function(mydata){
                            ...
                            if (userData.return == true){
                                $(".subfdbk").empty();
                                $('.subfdbk').html(customError('alert-success',userData.message));
                            }
                            else if(userData.return == false){
                                $(".subfdbk").empty();
                                $('.subfdbk').html(customError('alert-info',userData.message));
                            }
                        },
                        error:function(){
                            $(".subfdbk").empty();
...
function customError(Erclass,acerror) {
    var alert  = "";
    alert+='<div class="alert '+Erclass+' alert-dismissible fade show" role="alert">';
    alert+= '<strong>'+acerror+'</strong>';
